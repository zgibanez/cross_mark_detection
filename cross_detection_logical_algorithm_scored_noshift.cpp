...
vector<vector<Point>> FindSquares(Mat * thresh, Mat * frame)
{
	...
	for (i = 0; i < cnts.size(); i++) {
		...
		approxPolyDP(cnts[i], poligon, peri*0.1, 1);
	...
	cout << "FindSquares: The frame took " << time << " seconds to preccess" << endl;
...
int ScoreSquares(vector<vector<Point>> candidates, Mat* frame) {
	...
	e4 = getTickCount();
	time = (e4 - e3) / getTickFrequency();
	cout << "ScoreSquares: The frame took " << time << " seconds to preccess" << endl;
